...
# Coerce 'ipm' and 'kemiskinan' to numeric, converting "-" or other non-numeric to NaN
...
# ────────────────────────────────────────────────────────────────
pivot = df.groupby('geo_name_clean').agg(
    wilayah        = ('wilayah', 'first'),
    ipm_mean       = ('ipm',        'mean'),
    ipm_latest     = ('ipm',        'last'),
    kemiskinan_mean= ('kemiskinan', 'mean'),
    kemiskinan_latest=('kemiskinan','last'),
...
pivot['cluster']       = pivot['cluster_raw'].map(label_map)
pivot['cluster_color'] = pivot['cluster'].map(color_map)
...
for feat in geojson['features']:
    ...
    if nc in cdict:
        ...
        feat['properties'].update({
            ...
            'ipm_mean'         : round(float(c['ipm_mean']), 2),
            'ipm_latest'       : round(float(c['ipm_latest']), 2),
            'kemiskinan_mean'  : round(float(c['kemiskinan_mean']), 2),
            'kemiskinan_latest': round(float(c['kemiskinan_latest']), 2),
            'ipm_trend'        : round(float(c['ipm_trend']), 2),
            'kemiskinan_trend' : round(float(c['kemiskinan_trend']), 2),
            'wilayah'          : c['wilayah'],
        })
    else:
        feat['properties'].update({
            'cluster':'No Data','cluster_color':'#000000',
            'ipm_mean':None,'ipm_latest':None,
            'kemiskinan_mean':None,'kemiskinan_latest':None,
            'ipm_trend':None,'kemiskinan_trend':None,'wilayah':nc,
        })
...
for _, row in df.iterrows():
    ...
    if nm not in ts:
        ts[nm] = {
            ...
            'cluster_color': cdict.get(nm, {}).get('cluster_color', '#000000'),
        }
    ts[nm]['years'].append(yr)
    ts[nm]['ipm'].append(round(row['ipm'], 2) if pd.notna(row['ipm']) else None)
    ts[nm]['kemiskinan'].append(round(row['kemiskinan'], 2) if pd.notna(row['kemiskinan']) else None)
